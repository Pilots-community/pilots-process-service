 ...
 * EDC proxy calls are replaced by direct PATCH calls to the same server — the
 * cross-party routing is a network concern, not a process-service concern.
 *
 * Expected outbound calls in order:
 *   1. sendOrderCreated       → /erp/order-created          (on certiweight POST)
 *   2. sendMeasurementCreated → /erp/measurement-created    (on certiweight PATCH TRUCKER_ANNOUNCED)
 *   3. sendPurchaseVGM        → /tms/purchase-vgm           (on shipper PATCH MEASUREMENT_RECEIVED)
 *   4. sendVGMPurchased       → /erp/vgm-purchased          (on certiweight PATCH PURCHASE_CONFIRMED)
 */
...
class VgmE2ETest {
    ...
    void setUp() throws IOException {
        ...
        mockServer.start();
        // Pre-enqueue 200 OK for each of the 4 outbound ServiceTask POSTs
        for (int i = 0; i < 4; i++) {
    ...
    void fullVgmFlow_bothProcessesAdvanceCorrectlyAndComplete() throws Exception {
        ...
        // No ServiceTask fires yet — Shipper starts at receiveTask
        assertThat(mockServer.getRequestCount()).isEqualTo(0);

        // ── Step 2: Certiweight creates process instance ──────────────────────
        // sendOrderCreated ServiceTask fires automatically on start.
        ...
        // sendOrderCreated must have fired synchronously before POST returned
        ...
        // Certiweight process is now paused at waitTruckerAnnounced
        assertThat(mockServer.getRequestCount()).isEqualTo(1);

        // ── Step 2b: (Certiweight ERP → Shipper) advance waitOrderCreated ─────
        // Simulates the cross-EDC PATCH that Certiweight's ERP would send to Shipper.
        ...
        // ── Step 3: Trucker arrives — PATCH Certiweight directly ──────────────
        // This PATCH must set internalApiUrl for the immediately-following sendMeasurementCreated.

        ResponseEntity<Map> certPatch1 = patch(certId, "1",
                "TRUCKER_ANNOUNCED",
                Map.of(
                        "internalApiUrl", measurementCreatedUrl,
        ...
        assertThat(certPatch1.getBody().get("state")).isEqualTo("TRUCKER_ANNOUNCED");
        assertThat(certPatch1.getBody().get("version")).isEqualTo(2);

        // sendMeasurementCreated must have fired synchronously within this PATCH
        RecordedRequest req2 = mockServer.takeRequest(2, TimeUnit.SECONDS);
        assertThat(req2).as("sendMeasurementCreated POST").isNotNull();
        assertThat(req2.getPath()).isEqualTo("/erp/measurement-created");
        assertThat(req2.getBody().readUtf8()).contains("24500");
        ...
        assertThat(req3).as("sendPurchaseVGM POST").isNotNull();
        assertThat(req3.getPath()).isEqualTo("/tms/purchase-vgm");
        assertThat(req3.getBody().readUtf8()).contains("TCKU1234567");

        // Shipper is now paused at waitVGMPurchased
        assertThat(mockServer.getRequestCount()).isEqualTo(3);

        // ── Step 4: (Shipper TMS → Certiweight) advance waitPurchaseVGM ───────
        // This PATCH must set internalApiUrl for sendVGMPurchased.
        // After this, Certiweight's process runs through taskProcessPayment,
        // taskCreateCertificate (both no-op placeholders), sendVGMPurchased, then ENDS.

        ResponseEntity<Map> certPatch2 = patch(certId, "2",
                "PURCHASE_CONFIRMED",
                Map.of(
        ...
        assertThat(certPatch2.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(certPatch2.getBody().get("state")).isEqualTo("PURCHASE_CONFIRMED");
        assertThat(certPatch2.getBody().get("version")).isEqualTo(3);

        // sendVGMPurchased must have fired synchronously within this PATCH
        RecordedRequest req4 = mockServer.takeRequest(2, TimeUnit.SECONDS);
        assertThat(req4).as("sendVGMPurchased POST").isNotNull();
        assertThat(req4.getPath()).isEqualTo("/erp/vgm-purchased");
        assertThat(req4.getBody().readUtf8()).contains("CERT-2026-001");

        // Certiweight process is now COMPLETED (no more active executions)
        ...
        // ── Verify final state via GET (reads from history for completed processes)
        ...
        assertThat(getCert.getBody().get("version")).isEqualTo(3);
